...
class LinearClassifier():
    ...
    def multistart(self,X,y,  t, inertion, lambda_rec, lr, margin_present, eps,n_starts=10):#Инициализация весов через мультистарт
        ...
        for i in range(n_starts):
            self.fit(X[indexes],y[indexes],t=t, inertion=inertion, lambda_rec=lambda_rec, lr=lr, margin_present=margin_present, eps=eps)
            weights.append(self.W)
            accs=np.append(accs,self.count_accuracy(X,y))
        return weights[np.argmax(accs)]
        
    def fit(self,X,y,t=0.01, inertion=0.8, lambda_rec=0.8, lr=0.0001, fast_descent=False, margin_present=True, init_weights='random', eps=0.001):#Обучение через стохастический градиентный спуск с инерцией
